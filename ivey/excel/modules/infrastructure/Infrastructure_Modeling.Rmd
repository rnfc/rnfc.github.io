...
`r margin_note("__Note:__ Users cannot introduce changes directly to results or charts in the above sections, only to the assumptions or scenario analysis section.")` Finally, some common financial ratios are presented under 'Results' with minimum, maximum, and average values and their dates of occurrence. Similar to other results sheets, these ratios change if assumptions are changed or if scenario analysis is enabled. This section includes checks to validate the balance sheet and cash balances.
